...
whisper-typer: 3-thread macOS pipeline or legacy Linux loop.
  ...
  --> ring buffer        <-- frame queue -->         <-- ui queue
  --> frame queue        --> command queue -->        --> command queue
"""
...
class WhisperVoiceTyping:
    ...
    def _run_macos(self) -> None:
        ...
        self.state_machine.on_transition(lambda old, new: self._ui_queue.put(StateUpdate(new)))

        self._menubar = WhisperMenuBar(self.config, self._ui_queue, self._command_queue)

        self.running = True
        threading.Thread(target=self._processing_loop, daemon=True, name="processing").start()
            ...
            try:
                ...
                if not self._audio.is_active():
                    if state in (State.LISTENING, State.DETECTED, State.RECORDING):
                        tlog.warn("Audio stream inactive, restarting...")
                        if self._audio.restart_stream(): continue
    ...
    def _process_commands(self) -> None:
        ...
        while True:
            try: cmd = self._command_queue.get_nowait()
            ...
            if cmd == "start":
                try:
                    if self.state_machine.state in (State.IDLE, State.ERROR):
                        self.state_machine.transition(State.LISTENING)
                    if not self._audio.is_active(): self._audio.start()
                ...
                self._audio.stop()
                try: self.state_machine.transition(State.IDLE)
                except ValueError: self.state_machine.reset()
                self._ui_queue.put(StateUpdate(State.IDLE))

            elif cmd == "pause":
                self._audio.stop()
                try: self.state_machine.transition(State.PAUSED)
                except ValueError: pass
